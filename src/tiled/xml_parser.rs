...
pub fn parse_file(filesystem: &mut Filesystem, path: &Path) -> Result<Map, TiledError> {
    let file = filesystem
        .open(path)
...
/// parse it.
#[allow(dead_code)]
pub fn parse<R: Read>(filesystem: &mut Filesystem, reader: R) -> Result<Map, TiledError> {
